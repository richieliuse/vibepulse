...
typedef struct {
  ...
  int has_delta;
  int stale;
  /* Codex, Cursor, and Grok publish used percent. Their quota pages show
   * what is left, so the meter fill follows `pct` after that conversion and
   * today's usage stays in the caption instead of inside the remaining bar. */
  int meter_remaining;
} usage_card_view;
...
typedef struct {
  usage_card_view lanes[4];
} usage_cursor_page_view;
